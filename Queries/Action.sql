...
InventoryAnalysis AS (
    SELECT 
        ...
        CASE
            ...
            WHEN ts.quantityInStock > (ts.total_ordered_item / 18) * 12 AND ts.total_revenue > (ts.total_revenue / 18) * 12 THEN 'Overstocked'
...
SELECT 
    ...
    ia.inventory_status,
    CASE
		WHEN ia.inventory_status = 'Overstocked' AND (ia.productScale LIKE '%1:700%' OR ia.productScale LIKE '%1:72%' OR ia.productScale LIKE '%1:50%' OR ia.productScale LIKE '%1:32%') THEN 'Reduce'  -- reduce if scale is high
        WHEN ia.inventory_status = 'Understocked' THEN 'Increase'  -- increase if scale is low
        WHEN ia.inventory_status = 'Never ordered' AND (ia.quantityInStock > ia.good_stock_level_sales) THEN 'Reduce'
        ELSE 'Maintain'
    END AS action
